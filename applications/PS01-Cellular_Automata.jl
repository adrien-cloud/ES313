...
# ╔═╡ 148af541-fa34-4ee3-9633-027f9519a6be
function evolve_lava!(W, lava)
    size_W = size(W)
    lava_new = Dict{CartesianIndex{2}, Float64}()
    ...
    for (pos, q) in lava
        ...
        equal_heights = all(h .== hpos)
        if equal_heights
            share = q / (length(neigh) + 1)
            lava_new[pos] = get(lava_new, pos, 0.0) + share
            for k in neigh
                lava_new[k] = get(lava_new, k, 0.0) + share
            end
        else
            drops = hpos .- h
            tot = sum(drops)
            if tot == 0
                ...
                for (k, d) in zip(neigh, drops)
                    lava_new[k] = get(lava_new, k, 0.0) + q * d / tot
                end
            end
        end
    end
...
# ╔═╡ efe3c54e-bdbf-4619-84cf-b76b521f9825
let 
	iter=1000
	lava=Dict{CartesianIndex{2}, Float64}()
	
	for i in 1:iter
		lava[argmax(W)]=45
		lava[CartesianIndex(28,67)]=45
		evolve_lava!(W,lava)
	end



	lava_matrix=zeros(size(W))
	for (key,value) in lava
		lava_matrix[key]=value
	end
	lava_matrix
	heatmap(lava_matrix, c=:gist_earth, clims=(0,150), title="Lava map [m]", size=(400,400))
end
...
# ╔═╡ 1b712506-07f4-4874-8388-f48c353a2d7e
function ants_advance!(ants,home,food,food_marker,way_marker)
	for i in eachindex(ants)
    	prev_pos, pos, found_food = ants[i]

		size_food=size(food)
		I, J = Tuple(pos)
        directions = CartesianIndex{2}[]
        for (di, dj) in ((0, -1), (0, 1), (1, 0), (-1, 0))
            ni, nj = I + di, J + dj
            if 1 ≤ ni ≤ size_food[1] && 1 ≤ nj ≤ size_food[2] && CartesianIndex(ni, nj) != prev_pos
                push!(directions, CartesianIndex(ni, nj))
            end
        end
		
		if found_food
			food_marker[pos]+=1
			new_pos = rand([d for d in directions if way_marker[d] == maximum(way_marker[directions])])
			if new_pos==home
				deleteat!(ants,i)
				#@show "back home"
				break
			...
			way_marker[pos]+=1
			new_pos = rand([d for d in directions if food_marker[d] == maximum(food_marker[directions])])
			if food[new_pos]
				found_food=true
				#@show "yeeah"
			end
		...
		ants[i] = (pos, new_pos, found_food)
	end
	
end

# ╔═╡ ce485fd0-d9c4-4786-8ffb-9df52e09ee31
let
	iter=10000
	freq=10
	size=(64,64)
	way_marker=zeros(size)
	food_marker=zeros(size)
	food=zeros(Bool,size)
	food[20,20]=true
	home=CartesianIndex(Int64.(size./2))

	ants=Vector{Tuple{CartesianIndex{2},CartesianIndex{2},Bool}}()
	push!(ants,(home,home,false))

	for i in 1:iter
		if i % freq == 0
			push!(ants,(home,home,false))
		end
		ants_advance!(ants,home,food,food_marker,way_marker)
	end
	
	fig1=heatmap(food_marker, c=:gist_earth, clims=(0,150), title="Food pheromones map", size=(400,400))
	fig2=heatmap(way_marker, c=:gist_earth, clims=(0,150), title="Way pheromones map", size=(400,400))
...
# ╔═╡ 5b5ecb0d-a197-4adb-b20b-13493862cb50
begin

	@enum State q1 qy qn pn rn q2

	@enum Direction left right

	mutable struct TuringCell
	...
	rulez = Dict{Tuple{State, Bool}, Tuple{State, Union{Int64,Nothing}, Direction}}(
	    (q1, true)  => (pn, 0, right),
	    (q1, false) => (qy, 0, right),
	    (pn, true)  => (pn, nothing, right),
	    (pn, false) => (rn, 0, left),
	    (rn, true)  => (qn, nothing, left),
	    (rn, false) => (q2, 0, left),
	    (q2, true)  => (q2, nothing, left),
	    (q2, false) => (q1, 0, right)

	)

		function update!(arg::Vector, cell::TuringCell)
			if cell.c_state == q1 && cell.c_character != 0
				cell.c_comperator = cell.c_character
			end

			if cell.c_state == rn && cell.c_character != 0 && cell.c_character == cell.c_comperator
				cell.c_state = q2
				arg[cell.n] = 0
			end			

			next_state = rulez[(cell.c_state, (cell.c_character == 0 ? false : true))]
			cell.c_state = next_state[1]
			arg[cell.n] = next_state[2]== 0 ? 0 : arg[cell.n]			

			if next_state[3] == right
				cell.n += 1
			end
			if next_state[3] == left
				cell.n -= 1
			end	
			
			cell.c_character = arg[cell.n]						
			return cell, arg			

		end

		function check_palindrome(str::String)
			arg = Vector{Union{Char,Int64}}(undef,length(str)+1)
			arg[1:end-1] = collect(str)
			arg[end] = 0			
			cell = TuringCell(q1, arg[1],1,0)
			counter = 0

			while cell.c_state != qy && cell.c_state != qn && counter <= 25
				cell, arg = update!(arg, cell)	
				counter +=1
			end
	...
	user_input="kayak"
		try
			if check_palindrome(user_input) == qy
				println("It's a palindrome")
			elseif check_palindrome(user_input) == qn
				println("Too bad, no palindrome")
			end
		catch
			println("min 2 letters")
		end
